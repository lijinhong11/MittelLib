...
显示名与 lore 都会经过 [`ComponentUtils`](Utilities-zh#componentutils) 解析，
...
`MittelItem` 实现了 [`ReadWriteObject`](Configuration-zh#readwriteobject)：
...
> 请改用 [`CustomModelDataComponent`](Item-Components-zh)。
